...
def predict():
    try:
        ...
        if severity_level in ['High', 'Critical']:
            alert_message = f"🚨 ALERT: {attack_type} detected with {severity_level} severity."

        elif severity_level == 'Medium':
            alert_message = f"⚠ Suspicious traffic detected: {attack_type}."

        else:
            alert_message = "✔ Traffic appears normal."
            
        risk_score = float(data['anomaly_scores']) * severity_confidence
